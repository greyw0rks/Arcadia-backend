...
import { MAX_STAKE, MIN_STAKE } from "../../../server/difficulty";
...
export default function PlayPage() {
  ...
  async function begin() {
    ...
    const miniPay = typeof window !== "undefined" &&
      isMiniPay();

    // Pre-flight balance check on MiniPay — redirect to top-up before any on-chain call.
    if (miniPay) {
  ...
  async function finalize(sid: `0x${string}`) {
    try {
      ...
      // Win sound + confetti only on an actual win (> 1.0x); a neutral 1.0x just returns the stake.
